<template>
  <el-dialog v-model="show" title="添加菜单" width="500">
...
const formRef = ref()
const formData = ref<AddRoutesParams>({
  title: '',
  routesName: '',
  icon: '',
  fullPath: '',
...
const rules = reactive<FormRules<AddRoutesParams>>({
...
const getDictData = async () => {
  ...
  formData.value.type = routesTypeOptions.value ? routesTypeOptions.value[0].dictValue : ''
  formData.value.keepAlive = keepAliveOptions.value ? keepAliveOptions.value[0].dictValue : ''
}
...
const onSubmit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      const result = await addRoutesApi(formData.value)
      if (result.code === 200) {
        ElMessage({
